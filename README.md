...
```bash
curl -XPOST -d '{"name": "cpu", "url": "<prometheus_url>/api/v1/query?query=max(irate(container_cpu_system_seconds_total{container_label_tsuru_process_name=\"{process}\",container_label_tsuru_app_name=\"{app}\"}[1m]))*100&time=1475242592.104", "public": true}' -H "Content-Type: application/json" <autoscale-url>/datasource
```
