...
func (s *S) TestNewScale(c *check.C) {
	...
	c.Assert(al.Expression, check.Equals, fmt.Sprintf("data.aggregations.range.buckets[0].date.buckets[data.aggregations.range.buckets[0].date.buckets.length - 1].max.value %s %s", a.Operator, a.Value))
...
func (s *S) TestNew(c *check.C) {
	...
	c.Assert(al.Expression, check.Equals, fmt.Sprintf("!data.lock.Locked && data.units.length > %d", a.MinUnits))
...
func (s *S) TestEnableScaleDown(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	c.Assert(al.Name, check.Equals, alarmName)
	c.Assert(al.Expression, check.Equals, fmt.Sprintf("!data.lock.Locked && data.units.length > %d", minUnits))
